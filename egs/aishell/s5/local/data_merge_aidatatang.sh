...
rm -rf ${tar_trans_dir}
mkdir ${tar_trans_dir}
cat ${src_1_trans_file} | sed 's/S/W/' | sed 's/T0055/BAC009/' > "./temp.txt"
cat "./temp.txt" ${src_2_trans_file} | sort -u > ${tar_trans_file} || exit 1;
...
for dir in dev test train; do
    ...
    for file in `cat ${src_wav_dir}/${dir}_wav.txt`; do
        # file=/home/rd/caiyueliang/data/aidatatang/aidatatang_200zh/corpus/dev/G0726/T0055G0726S0428.wav
        ...
        file_id=`echo -n ${file_id:5} | sed 's/S/W/'`
        ...
        cp -r ${file} ${tar_wav_dir}/${dir}/${dir_name}/${prefix}${file_id}".wav" || exit 1;
    done
