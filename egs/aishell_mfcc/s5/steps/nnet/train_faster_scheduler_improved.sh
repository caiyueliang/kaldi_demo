...
for iter in $(seq -w ${max_iters}); do
  ...
  # ==========================================================================================================
  # 在这里加生成子集的代码。用生成新的feats.scp替换train.scp。生成新的目录替换${feats_tr}中的旧目录，
  ...
  # echo "[train_faster_scheduler]  feats_tr: ""${feats_tr}"
  dir_name=$(basename ${feats_dir})
  feats_sub_tr=`echo "${feats_tr}" | sed "s/${dir_name}/${dir_name}_sub_${iter}/g"`
  # echo "[train_faster_scheduler]  feats_tr: ""${feats_sub_tr}"

  # ==========================================================================================================
  # shuffle train.scp 打乱s5/exp/tri7b_DFSMN_L/train.scp的顺序
