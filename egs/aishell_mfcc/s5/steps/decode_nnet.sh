...
if [ ! -z "$blocksoftmax_dims" ]; then
  ...
  echo "[dim_total] "${dim_total}
  echo "[dim_block] "${dim_block}
...
echo "[decode] 4 ================================ "
echo "[decode]  feature_transform: "${feature_transform}
echo "[decode] class-frame-counts: "${class_frame_counts}
echo "[decode]            use_gpu: "${use_gpu}
echo "[decode]               nnet: "${nnet}
echo "[decode]              feats: "${feats}
echo "[decode]      thread_string: "${thread_string}
echo "[decode]         min_active: "${min_active}
echo "[decode]         max_active: "${max_active}
echo "[decode]            max_mem: "${max_mem}
echo "[decode]               beam: "${beam}
echo "[decode]       lattice_beam: "${lattice_beam}
echo "[decode]               acwt: "${acwt}
echo "[decode]  word-symbol-table: "${graphdir}/words.txt
echo "[decode]              model: "${model}
echo "[decode]           HCLG.fst: "${graphdir}/HCLG.fst

# Run the decoding in the queue,
if [ $stage -le 0 ]; then
  $cmd --num-threads $((num_threads+1)) JOB=1:${nj} ${dir}/log/decode.JOB.log \
    nnet-forward ${nnet_forward_opts} --feature-transform=${feature_transform} \
