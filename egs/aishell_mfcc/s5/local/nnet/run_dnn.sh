...
echo "[run_dnn.sh]    mini_batch: "${mini_batch}
echo "[run_dnn.sh]          acwt: "${acwt}
echo "[run_dnn.sh]  train_faster: "${train_faster}
echo "[run_dnn.sh] ===================================="
...
if [ "${feats_gen}" -eq "1" ]; then
    echo "[run_dnn.sh] Re-generate features data ..."
    alidir=${gmmdir}_sp_ali
    local/data_transformer.sh --cmd "${train_cmd}" --nj ${nj} --feats_type ${feats_type} \
        --train_set ${train_set} --test_set ${test_set} --dev_set ${dev_set} \
        --speed_perturb ${speed_perturb} --volume_perturb ${volume_perturb} \
        --reverberate_data ${reverberate_data} --augment_data ${augment_data} \
        --align_data ${align_data} ${gmmdir} ${alidir} || exit 1;
#    # 添加音速扰动
    ...
    if [ ! -z ${nnet_init} ]; then
        ...
        if [ "${train_faster}" -eq "0" ]; then
            ${cuda_cmd} ${dir}/train_faster_nnet.log \
                ...
                --train-tool-opts "--minibatch-size=${mini_batch}" \
                --nnet_init ${nnet_init} \
